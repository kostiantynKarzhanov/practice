...
public class LoggerAspect {
    ...
    public Mono<ResponseEntity<PaymentDTO>> log(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        this.logger.info(
                "payment received from the client and is going to be send further to Payment Service"
        );

        Mono<ResponseEntity<PaymentDTO>> returnedValue =
                (Mono<ResponseEntity<PaymentDTO>>) proceedingJoinPoint.proceed();

//        this.logger.info("Payment Service response received and is going to be send further to client: " + returnedValue.getBody());
        this.logger.info("Payment Service response received and is going to be send further to client");
